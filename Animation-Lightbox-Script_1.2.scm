...
;; Project Lightbox:
;; Part of series of GIMP scripts to better emulate animating
;; traditionally with paper
...
;; GIMP plugin website.
;;
;; Exporting to Video will require the GIMP Animation Package (GAP)
;; which can be downloaded from the GIMP plugin registry:
;; http://registry.gimp.org/node/3700
;;
;; Highly recommend assigning shortcuts to speed up workflow.

;;; INSTALL
;; Place all scripts in ".gimp-2.6/scripts", this should be in your
;; main user folder for both LINUX and WINDOWS. Restart GIMP or If
;; already running go to Filters - Script-Fu - Refresh Scripts on the
;; menu bar.

;;; Known Issues
;; - Cycle-up / Cycle-down can sometimes crash script-fu if pressed
;;   together. Simply save files and restart GIMP.
;; - Failed script errors will occur if layers are not within naming
;;   convention.  Generally can be avoided by using plugins to rename.
...
(script-fu-menu-register "lbox-layer-up"
                         "<Image>/_Animate/_Tools/Cycle _Frames/")
...
(script-fu-menu-register "lbox-onionskin-down"
                         "<Image>/_Animate/_Tools/Onion-Ski_n/")

(script-fu-register
 "lbox-onionskin-up"
 "Cycle Onion Skinning _Up"
...
(script-fu-menu-register "opacity-update-all"
                         "<Image>/_Animate/_Tools/Op_acity/")

(script-fu-register
 "opacity-update-frames"
 "Change Opacity on All _Frames"
...
(script-fu-menu-register "lbox-retime-double-exposure"
                         "<Image>/_Animate/_Tools/_Timing/")

(script-fu-register
 "lbox-retime-single-exposure"
 "Retime Frame to 'on _1s'"
...
(script-fu-register
 ...
 "Retime Frame to _Custom Exposure"
